 ...
 *   - Padre Pio footer
 *
 * Output: 1080×1350 PNG (Instagram-friendly 4:5 aspect ratio)
...
export function generateParchmentImage(data: ParchmentCardData): string {
  ...
  ctx.font = 'italic 42px "EB Garamond", Georgia, "Times New Roman", serif';
  ctx.fillStyle = '#3d2b1a';
  ctx.textAlign = 'center';

  const prayerLines = wrapText(ctx, `\u201C${data.devotionalText}\u201D`, CARD_W - 160);
  const lineHeight = 68;

  // Center the prayer text vertically in the available space
  const prayerBlockHeight = prayerLines.length * lineHeight;
  const availableSpace = CARD_H - yPos - 350; // reserve space for divider + scripture + footer
  const prayerStartY = yPos + Math.max(0, (availableSpace - prayerBlockHeight) / 2);
  ...
  // ── Cross divider ───────────────────────────────────────────────────────
  const dividerY = prayerStartY + prayerBlockHeight + 50;
  ...
  ctx.textAlign = 'center';
  ctx.fillText('✝', CARD_W / 2, dividerY + 10);
  ...
  for (let i = 0; i < scriptLines.length; i++) {
    ctx.fillText(scriptLines[i], CARD_W / 2, scriptY + i * 40);
  }
  ...
  ctx.letterSpacing = '6px';
  ctx.textAlign = 'center';
  ctx.fillText('PADRE PIO  ·  PRAY, HOPE AND DON\'T WORRY', CARD_W / 2, CARD_H - 50);
  ctx.letterSpacing = '0px';
